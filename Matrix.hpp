...
std::string multiplyDoubleStrings(const std::string &s1, const std::string &s2);
...
std::string multiplyDoubleStrings(const std::string &s1, const std::string &s2) {
    ParsedNumber num1 = parseNumber(s1);
    ParsedNumber num2 = parseNumber(s2);
    std::string whole1 = num1.integer + num1.fraction;
    std::string whole2 = num2.integer + num2.fraction;
    std::vector<int> result(whole1.size() + whole2.size(), 0);
    for (int i = whole1.size() - 1; i >= 0; i--) {
        for (int j = whole2.size() - 1; j >= 0; j--) {
            int mul = (whole1[i] - '0') * (whole2[j] - '0');
            int p1 = i + j, p2 = i + j + 1;
    ...
    for (int digit : result) {
        if (digit != 0 || nonZeroFound) {
            resultStr.push_back('0' + digit);
            nonZeroFound = true;
        }
    }
    if (!nonZeroFound)
        return "0";
    int totalDecimal = num1.fraction.size() + num2.fraction.size();
    if (resultStr.size() <= static_cast<size_t>(totalDecimal)) {
        resultStr = std::string(totalDecimal - resultStr.size() + 1, '0') + resultStr;
    }
    resultStr.insert(resultStr.end() - totalDecimal, '.');
    if (num1.sign * num2.sign < 0)
